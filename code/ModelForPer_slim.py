...
class PersonalLLM_Slim(nn.Module):
    ...
    - Keeps LLM frozen
    - Adds Session-Aware Transformer placeholder
    - Adds Graph Embedding table placeholder
    - Adds Gated Cross-Attention fusion
    """
    ...
    def __init__(self, llm_model, emb_model, max_input_len, max_new_len, task_id):
        ...
        # Precomputed long-term history embeddings (train/dev)
        ...
        # Graph embeddings table placeholder (precomputed offline)
        try:
            graph_emb = torch.cat(
                [torch.zeros(1, self.emb_emb_size),
        ...
        self.session_encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(
                d_model=self.emb_emb_size, nhead=4, batch_first=True
            ), num_layers=2
        )
    ...
    def obtain_profile_emb(self, his_id, task_embs):
        ...
        his_embs = (self.his_train_emb_table(his_id)
                    if self.training else self.his_dev_emb_table(his_id))
    ...
    def forward(
        ...
        if session_input is not None:
            session_embs = self.session_encoder(session_input)[:, -1, :]

        graph_embs = None
        if self.graph_emb_table is not None and graph_ids is not None:
            graph_embs = self.graph_emb_table(graph_ids)

        # Combine available personalization signals
        user_embs_list = [profile_embs]
        if session_embs is not None:
            user_embs_list.append(session_embs)
        if graph_embs is not None:
            user_embs_list.append(graph_embs)
        combined_user_embs = torch.cat(user_embs_list, dim=1).unsqueeze(1)

        # Fuse embeddings with gated cross-attention
