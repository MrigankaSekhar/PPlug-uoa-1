...
def train_model(model_args, data_args, training_args):
    ...
    # Dataset subsetting for quick testing (optional)
    class FixedLengthSubset(Subset):
        def __len__(self):
            return len(self.indices)   # proper length of subset
    ...
    train_size = min(1000, len(sub_train_dataset))
    eval_size = min(500, len(sub_eval_dataset))
...
if __name__ == '__main__':
    ...
    # 🔹 Disable safetensors saving to avoid tied-weights crash
