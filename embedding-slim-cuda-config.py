...
def get_embedding_streaming(sentences, save_path, max_length=512, clean_temp=True):
    """
    Compute embeddings in batches, save each batch to .pt in OUTPUT_DIR chunks folder,
    and merge at the end without loading all batches into RAM.
    ...
    chunk_dir = os.path.join(os.path.dirname(save_path), "chunks")
    os.makedirs(chunk_dir, exist_ok=True)

    # Resume check: find highest existing batch
    existing_batches = sorted(glob.glob(os.path.join(chunk_dir, "batch_*.pt")))
    if existing_batches:
        last_batch_idx = max(int(os.path.basename(bf).split("_")[1].split(".")[0]) for bf in existing_batches)
        start_batch = last_batch_idx + 1
        print(f"[resume] Found {len(existing_batches)} batch files. Resuming at batch {start_batch}.")
    else:
        start_batch = 0
        print("[resume] No existing batch files. Starting from batch 0.")

    total_batches = len(sentences) // batch_size + (1 if len(sentences) % batch_size else 0)
    start_time = time.time()

    # ---- Process batches from resume point ----
    for batch_idx in tqdm(range(start_batch, total_batches), desc="Embedding batches", initial=start_batch, total=total_batches):
        ...
        batch_path = os.path.join(chunk_dir, f"batch_{batch_idx}.pt")
    ...
    # ---- Merge step ----
    print("[embedding] Merging batches to final tensor...")
    batch_files = sorted(glob.glob(os.path.join(chunk_dir, "batch_*.pt")))
    final_parts = []
    for bf in batch_files:
        final_parts.append(torch.load(bf, map_location="cpu"))
...
def merge_batches_streaming(chunk_dir, save_path):
    """
    Merge all batch .pt files into one tensor and save to save_path
    ...
    for idx, bf in enumerate(batch_files):
        ...
        if final_tensor is None:
            ...
            # Concatenate safely batch-by-batch
            final_tensor = torch.cat([final_tensor, batch], dim=0)
        del batch

    torch.save(final_tensor.contiguous(), save_path)
